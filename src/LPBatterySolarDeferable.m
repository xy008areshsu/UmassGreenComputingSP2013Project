...
% 9 * T = 192 varibles: see above
count = T / preemptibleLoads(1, 2);
Aeq = zeros(2 * T + count, 9 * T);
beq = zeros(2 * T + count, 1);
...
for i = 1 : T
    ...
    Aeq(i, i + 8 * T) = -1;                 % -preemptibleLoad_t
...
k = 0;
for i = 2 * T + 1 : 2 * T + count
    for j = 0 : period - 1
        Aeq(i, i - 2 * T + 8 * T + k * period + j) = 1;
    end
    k = k + 1;
...
preemptibleLoads = reshape(x(8 * T + 1 : 9 * T), T, 1);
cost = cost / 100;     % convert from cents to dollars

%% =======================Plot Results and Write to File===================
originalPrice = sum(nonDeferLoad.*GridCost) / 100 + sum(56 / 8 * GridCost(16:24)) / 100;
...
scheduleSolarBatt = [BattGreen, BattGrid, LoadBatt, LoadGrid, Grid, LoadGreen, NetGreen, nonDeferLoad];
